...
const CLIENT_INFO = { name: 'sf-mcp-proxy-cli', version: '0.1.0' };
...
export async function connect(serverUrl: string, opts: ConnectOptions = {}): Promise<ConnectResult> {
  ...
  if (!interactive) {
    ...
    if (!tok?.access_token && !tok?.refresh_token) {
      throw new Error(
        `No cached token for ${serverUrl}. Run \`sf-mcp-proxy login\` for this server first (--no-login can't open a browser).`,
      );
    }
  }
  ...
  try {
    ...
    if (!interactive) {
      throw new Error('Cached credentials are no longer valid. Run `sf-mcp-proxy login` for this server.');
    }
